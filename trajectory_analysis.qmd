...
```{python}
heat = summary.pivot('release_id', 'segment', 'arrivals')
...
for seg, seg_df in example.groupby('segment'):
    plt.plot(seg_df['lon'], seg_df['lat'], label=f'Segment {seg}')
plt.legend(title='Gaza Segment')
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.title(f'Trajectories on {example_date.date()}')
plt.show()
```
...
```{python}
prod = summary.groupby('segment')['arrivals'].mean()
plt.figure(figsize=(8,4))
prod.plot.bar(color='skyblue')
plt.xlabel('Gaza Segment')
